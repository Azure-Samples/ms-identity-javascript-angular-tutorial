...
Function ConfigureApplications
{
   ...
   $serviceAadApplication = New-MgApplication -DisplayName "ProfileAPI" `
                                                       -Web `
                                                       @{ `
                                                         } `
                                                         -Api `
    ...
    Write-Host "Successfully registered and configured that app registration for 'ProfileAPI' at `n $servicePortalUrl" -ForegroundColor Green 
   ...
   $clientAadApplication = New-MgApplication -DisplayName "ProfileSPA" `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/ProfileSPA")
    ...
    # print the registered app portal URL for any further navigation
    Write-Host "Successfully registered and configured that app registration for 'ProfileSPA' at `n $clientPortalUrl" -ForegroundColor Green 
    ...
    # $configFile = $pwd.Path + "\..\API\ProfileAPI\appsettings.json"
    $configFile = $(Resolve-Path ($pwd.Path + "\..\API\ProfileAPI\appsettings.json"))
    
    $dictionary = @{ "Enter the client Secret" = $serviceAppKey;"Enter the client ID (aka 'Application ID')" = $serviceAadApplication.AppId;"Enter the tenant ID" = $tenantId };
