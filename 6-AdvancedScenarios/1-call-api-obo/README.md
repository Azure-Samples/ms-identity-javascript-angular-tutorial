---
page_type: sample
description: This sample demonstrates an Angular single-page application (SPA) which lets a user authenticate with their Microsoft Entra tenant and obtains an access token to call an ASP.NET Core web API, protected by Microsoft Entra ID.
languages:
 - typescript
 - csharp
products:
...
This sample demonstrates an Angular single-page application (SPA) which lets a user authenticate with their Microsoft Entra tenant and obtains an [access token](https://aka.ms/access-tokens) to call an ASP.NET Core web API, protected by [Microsoft Entra ID](https://azure.microsoft.com/services/active-directory/).
...
- Once the user authenticates, **ProfileSPA** obtains an [access token](https://aka.ms/access-tokens) from Microsoft Entra ID.
...
1. Find the key `Enter the client Secret` and replace the existing value with the generated secret that you saved during the creation of `ProfileAPI` copied from the Microsoft admin center.
1. Find the key `Enter the client ID (aka 'Application ID')` and replace the existing value with the application ID (clientId) of `ProfileAPI` app copied from the Microsoft admin center.
1. Find the key `Enter the tenant ID` and replace the existing value with your Microsoft Entra tenant/directory ID.
...
1. Find the key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of `ProfileSPA` app copied from the Microsoft admin center.
1. Find the key `Enter_the_Tenant_Info_Here` and replace the existing value with your Microsoft Entra tenant/directory ID.
1. Find the key `Enter_the_Web_Api_Application_Id_Here` and replace the existing value with the application ID (clientId) of `ProfileAPI` app copied from the Microsoft admin center.
...
1. In the [Microsoft admin center](https://portal.azure.com), navigate to your `ProfileAPI` app registration, and select the **Manifest** blade.
